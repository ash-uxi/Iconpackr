...
/**
 * PiPurseBagCheckContrast icon from the contrast style in general category.
 */
interface PiPurseBagCheckContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiPurseBagCheckContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'purse-bag-check icon',
  ...props
}: PiPurseBagCheckContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
