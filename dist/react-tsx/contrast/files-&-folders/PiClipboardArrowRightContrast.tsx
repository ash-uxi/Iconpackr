...
/**
 * PiClipboardArrowRightContrast icon from the contrast style in files-&-folders category.
 */
interface PiClipboardArrowRightContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardArrowRightContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-arrow-right icon',
  ...props
}: PiClipboardArrowRightContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
