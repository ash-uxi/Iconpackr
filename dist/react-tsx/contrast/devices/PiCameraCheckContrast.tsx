...
/**
 * PiCameraCheckContrast icon from the contrast style in devices category.
 */
interface PiCameraCheckContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiCameraCheckContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'camera-check icon',
  ...props
}: PiCameraCheckContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
  ...
  <path d="m9 13.313 2.341 2.339a15 15 0 0 1 4.558-4.936"/>
    </svg>
  );
}
