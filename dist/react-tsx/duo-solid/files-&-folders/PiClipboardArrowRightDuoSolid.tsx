...
/**
 * PiClipboardArrowRightDuoSolid icon from the duo-solid style in files-&-folders category.
 */
interface PiClipboardArrowRightDuoSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardArrowRightDuoSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-arrow-right icon',
  ...props
}: PiClipboardArrowRightDuoSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
