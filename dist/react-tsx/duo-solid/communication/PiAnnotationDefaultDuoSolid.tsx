...
/**
 * PiAnnotationDefaultDuoSolid icon from the duo-solid style in communication category.
 */
interface PiAnnotationDefaultDuoSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiAnnotationDefaultDuoSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'annotation-default icon',
  ...props
}: PiAnnotationDefaultDuoSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
