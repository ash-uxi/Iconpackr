...
/**
 * PiClipboardCancelStroke icon from the stroke style in files-&-folders category.
 */
interface PiClipboardCancelStrokeProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardCancelStroke({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-cancel icon',
  ...props
}: PiClipboardCancelStrokeProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
