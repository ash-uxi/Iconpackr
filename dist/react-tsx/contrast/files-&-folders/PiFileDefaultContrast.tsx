...
/**
 * PiFileDefaultContrast icon from the contrast style in files-&-folders category.
 */
interface PiFileDefaultContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFileDefaultContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'file-default icon',
  ...props
}: PiFileDefaultContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
