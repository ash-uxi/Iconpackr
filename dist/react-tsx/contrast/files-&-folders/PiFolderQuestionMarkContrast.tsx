...
/**
 * PiFolderQuestionMarkContrast icon from the contrast style in files-&-folders category.
 */
interface PiFolderQuestionMarkContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFolderQuestionMarkContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'folder-question-mark icon',
  ...props
}: PiFolderQuestionMarkContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
