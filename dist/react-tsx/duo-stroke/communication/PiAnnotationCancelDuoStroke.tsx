...
/**
 * PiAnnotationCancelDuoStroke icon from the duo-stroke style in communication category.
 */
interface PiAnnotationCancelDuoStrokeProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiAnnotationCancelDuoStroke({
  size = 24,
  color,
  className,
  ariaLabel = 'annotation-cancel icon',
  ...props
}: PiAnnotationCancelDuoStrokeProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
  ...
  <path d="M9.5 13.5L12 11M12 11L14.5 8.5M12 11L9.5 8.5M12 11L14.5 13.5" stroke={color || "currentColor"} strokeWidth="2" fill="none"/>
    </svg>
  );
}
