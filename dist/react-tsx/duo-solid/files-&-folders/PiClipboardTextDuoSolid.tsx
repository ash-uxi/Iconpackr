...
/**
 * PiClipboardTextDuoSolid icon from the duo-solid style in files-&-folders category.
 */
interface PiClipboardTextDuoSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardTextDuoSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-text icon',
  ...props
}: PiClipboardTextDuoSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
