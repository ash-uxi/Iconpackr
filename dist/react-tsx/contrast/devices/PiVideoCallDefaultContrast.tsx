...
/**
 * PiVideoCallDefaultContrast icon from the contrast style in devices category.
 */
interface PiVideoCallDefaultContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiVideoCallDefaultContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'video-call-default icon',
  ...props
}: PiVideoCallDefaultContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
