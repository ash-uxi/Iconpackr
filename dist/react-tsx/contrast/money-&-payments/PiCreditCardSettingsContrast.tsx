...
/**
 * PiCreditCardSettingsContrast icon from the contrast style in money-&-payments category.
 */
interface PiCreditCardSettingsContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiCreditCardSettingsContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'credit-card-settings icon',
  ...props
}: PiCreditCardSettingsContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
