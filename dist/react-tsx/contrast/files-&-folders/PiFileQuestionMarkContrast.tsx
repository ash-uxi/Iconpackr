...
/**
 * PiFileQuestionMarkContrast icon from the contrast style in files-&-folders category.
 */
interface PiFileQuestionMarkContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFileQuestionMarkContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'file-question-mark icon',
  ...props
}: PiFileQuestionMarkContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
