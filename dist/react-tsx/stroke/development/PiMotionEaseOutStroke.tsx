...
/**
 * PiMotionEaseOutStroke icon from the stroke style in development category.
 */
interface PiMotionEaseOutStrokeProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiMotionEaseOutStroke({
  size = 24,
  color,
  className,
  ariaLabel = 'motion-ease-out icon',
  ...props
}: PiMotionEaseOutStrokeProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
      ...
      {...props}
    >
      <path d="M17 7C10 7 7 17 7 17" stroke="currentColor" strokeWidth="2" fill="none"/>
