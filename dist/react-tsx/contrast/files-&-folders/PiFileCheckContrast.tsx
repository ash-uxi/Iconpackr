...
/**
 * PiFileCheckContrast icon from the contrast style in files-&-folders category.
 */
interface PiFileCheckContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFileCheckContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'file-check icon',
  ...props
}: PiFileCheckContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
