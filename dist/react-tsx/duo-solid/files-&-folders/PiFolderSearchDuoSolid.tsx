...
/**
 * PiFolderSearchDuoSolid icon from the duo-solid style in files-&-folders category.
 */
interface PiFolderSearchDuoSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFolderSearchDuoSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'folder-search icon',
  ...props
}: PiFolderSearchDuoSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
