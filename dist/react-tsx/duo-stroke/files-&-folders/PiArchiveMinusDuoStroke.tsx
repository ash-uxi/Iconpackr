 ...
 */
interface PiArchiveMinusDuoStrokeProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiArchiveMinusDuoStroke({
  size = 24,
  color,
  className,
  ariaLabel = 'archive-minus icon',
  ...props
}: PiArchiveMinusDuoStrokeProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
  ...
  <path d="M9 14.5H15" stroke={color || "currentColor"} strokeWidth="2" fill="none"/>
    </svg>
  );
}
