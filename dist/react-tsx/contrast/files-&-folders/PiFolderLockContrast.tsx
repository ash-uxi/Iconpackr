...
/**
 * PiFolderLockContrast icon from the contrast style in files-&-folders category.
 */
interface PiFolderLockContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFolderLockContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'folder-lock icon',
  ...props
}: PiFolderLockContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
