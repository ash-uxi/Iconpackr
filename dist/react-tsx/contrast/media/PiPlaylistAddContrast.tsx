...
/**
 * PiPlaylistAddContrast icon from the contrast style in media category.
 */
interface PiPlaylistAddContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiPlaylistAddContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'playlist-add icon',
  ...props
}: PiPlaylistAddContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
      ...
      {...props}
    >
      <path d="M12 19v-3m0 0v-3m0 3H9m3 0h3"/>
