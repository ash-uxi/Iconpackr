...
/**
 * PiClipboardPlusContrast icon from the contrast style in files-&-folders category.
 */
interface PiClipboardPlusContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardPlusContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-plus icon',
  ...props
}: PiClipboardPlusContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
