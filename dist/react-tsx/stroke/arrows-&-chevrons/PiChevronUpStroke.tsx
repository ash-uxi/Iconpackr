import React from 'react';

/**
 * PiChevronUpStroke icon from the stroke style in arrows-&-chevrons category.
 */
interface PiChevronUpStrokeProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
}

export default function PiChevronUpStroke({
  size = 24,
  color,
  className,
  ariaLabel = 'chevron-up icon',
  ...props
}: PiChevronUpStrokeProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      fill="none"
      stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{color: color || "currentColor"}}
      
      role="img"
      aria-label={ariaLabel}
      {...props}
    >
      <path d="M8 14C9.06206 12.5381 10.3071 11.2287 11.7021 10.1058C11.8774 9.96473 12.1226 9.96473 12.2979 10.1058C13.6929 11.2287 14.9379 12.5381 16 14" stroke="currentColor" strokeWidth="2" fill="none"/>
    </svg>
  );
}
