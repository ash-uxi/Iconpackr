...
/**
 * PiFileExclamationMarkContrast icon from the contrast style in files-&-folders category.
 */
interface PiFileExclamationMarkContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFileExclamationMarkContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'file-exclamation-mark icon',
  ...props
}: PiFileExclamationMarkContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
