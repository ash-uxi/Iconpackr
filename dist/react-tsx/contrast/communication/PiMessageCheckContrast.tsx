...
/**
 * PiMessageCheckContrast icon from the contrast style in communication category.
 */
interface PiMessageCheckContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiMessageCheckContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'message-check icon',
  ...props
}: PiMessageCheckContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
  ...
  <path d="m8.5 10.309 2.341 2.339c1.148-2.008 2.752-3.703 4.659-5.005"/>
    </svg>
  );
}
