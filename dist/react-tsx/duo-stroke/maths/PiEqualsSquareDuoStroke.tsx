 ...
 */
interface PiEqualsSquareDuoStrokeProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiEqualsSquareDuoStroke({
  size = 24,
  color,
  className,
  ariaLabel = 'equals-square icon',
  ...props
}: PiEqualsSquareDuoStrokeProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
  ...
  <path d="M9 10H15M9 14H15" stroke={color || "currentColor"} strokeWidth="2" fill="none"/>
    </svg>
  );
}
