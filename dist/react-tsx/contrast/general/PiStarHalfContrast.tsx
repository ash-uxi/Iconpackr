...
/**
 * PiStarHalfContrast icon from the contrast style in general category.
 */
interface PiStarHalfContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiStarHalfContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'star-half icon',
  ...props
}: PiStarHalfContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
