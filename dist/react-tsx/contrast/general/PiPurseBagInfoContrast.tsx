...
/**
 * PiPurseBagInfoContrast icon from the contrast style in general category.
 */
interface PiPurseBagInfoContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiPurseBagInfoContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'purse-bag-info icon',
  ...props
}: PiPurseBagInfoContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
