 ...
 */
interface PiThermometerMinusStrokeProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiThermometerMinusStroke({
  size = 24,
  color,
  className,
  ariaLabel = 'thermometer-minus icon',
  ...props
}: PiThermometerMinusStrokeProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
  ...
  <path d="M2 7H7.99999" stroke="currentColor" strokeWidth="2" fill="none"/>
    </svg>
  );
}
