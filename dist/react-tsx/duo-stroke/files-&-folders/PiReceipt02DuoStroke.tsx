 ...
 */
interface PiReceipt02DuoStrokeProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiReceipt02DuoStroke({
  size = 24,
  color,
  className,
  ariaLabel = 'receipt-02 icon',
  ...props
}: PiReceipt02DuoStrokeProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
  ...
  <path d="M8 7H16M8 11H16M8 15H12" stroke={color || "currentColor"} strokeWidth="2" fill="none"/>
    </svg>
  );
}
