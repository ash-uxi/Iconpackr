...
/**
 * PiSidebarRightOpenStroke icon from the stroke style in general category.
 */
interface PiSidebarRightOpenStrokeProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiSidebarRightOpenStroke({
  size = 24,
  color,
  className,
  ariaLabel = 'sidebar-right-open icon',
  ...props
}: PiSidebarRightOpenStrokeProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
