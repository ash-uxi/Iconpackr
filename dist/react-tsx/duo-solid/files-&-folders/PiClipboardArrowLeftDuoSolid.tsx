...
/**
 * PiClipboardArrowLeftDuoSolid icon from the duo-solid style in files-&-folders category.
 */
interface PiClipboardArrowLeftDuoSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardArrowLeftDuoSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-arrow-left icon',
  ...props
}: PiClipboardArrowLeftDuoSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
