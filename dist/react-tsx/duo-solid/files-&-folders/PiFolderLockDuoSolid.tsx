...
/**
 * PiFolderLockDuoSolid icon from the duo-solid style in files-&-folders category.
 */
interface PiFolderLockDuoSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFolderLockDuoSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'folder-lock icon',
  ...props
}: PiFolderLockDuoSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
