...
/**
 * PiPresentationTrendlineDownDuoSolid icon from the duo-solid style in chart-&-graph category.
 */
interface PiPresentationTrendlineDownDuoSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiPresentationTrendlineDownDuoSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'presentation-trendline-down icon',
  ...props
}: PiPresentationTrendlineDownDuoSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
