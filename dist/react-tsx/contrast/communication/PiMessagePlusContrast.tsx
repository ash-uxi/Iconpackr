...
/**
 * PiMessagePlusContrast icon from the contrast style in communication category.
 */
interface PiMessagePlusContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiMessagePlusContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'message-plus icon',
  ...props
}: PiMessagePlusContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
