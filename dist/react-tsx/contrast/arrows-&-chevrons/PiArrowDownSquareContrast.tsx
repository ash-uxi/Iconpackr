...
/**
 * PiArrowDownSquareContrast icon from the contrast style in arrows-&-chevrons category.
 */
interface PiArrowDownSquareContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiArrowDownSquareContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'arrow-down-square icon',
  ...props
}: PiArrowDownSquareContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
