...
/**
 * PiClipboardArrowUpContrast icon from the contrast style in files-&-folders category.
 */
interface PiClipboardArrowUpContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardArrowUpContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-arrow-up icon',
  ...props
}: PiClipboardArrowUpContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
