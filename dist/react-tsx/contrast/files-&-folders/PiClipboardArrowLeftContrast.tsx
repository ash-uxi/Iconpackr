...
/**
 * PiClipboardArrowLeftContrast icon from the contrast style in files-&-folders category.
 */
interface PiClipboardArrowLeftContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardArrowLeftContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-arrow-left icon',
  ...props
}: PiClipboardArrowLeftContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
