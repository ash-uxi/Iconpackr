...
/**
 * PiPlaylistCheckContrast icon from the contrast style in media category.
 */
interface PiPlaylistCheckContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiPlaylistCheckContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'playlist-check icon',
  ...props
}: PiPlaylistCheckContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
  ...
  <path d="m8.5 16.14 2.341 2.34c1.148-2.008 2.752-3.703 4.659-5.005"/>
    </svg>
  );
}
