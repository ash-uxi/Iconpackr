...
/**
 * PiNotificationBellAddContrast icon from the contrast style in alerts category.
 */
interface PiNotificationBellAddContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiNotificationBellAddContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'notification-bell-add icon',
  ...props
}: PiNotificationBellAddContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
