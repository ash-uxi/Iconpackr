...
/**
 * PiFolderArrowDownDuoSolid icon from the duo-solid style in files-&-folders category.
 */
interface PiFolderArrowDownDuoSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFolderArrowDownDuoSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'folder-arrow-down icon',
  ...props
}: PiFolderArrowDownDuoSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
