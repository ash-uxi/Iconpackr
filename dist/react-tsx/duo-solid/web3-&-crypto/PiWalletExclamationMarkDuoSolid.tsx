...
/**
 * PiWalletExclamationMarkDuoSolid icon from the duo-solid style in web3-&-crypto category.
 */
interface PiWalletExclamationMarkDuoSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiWalletExclamationMarkDuoSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'wallet-exclamation-mark icon',
  ...props
}: PiWalletExclamationMarkDuoSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
