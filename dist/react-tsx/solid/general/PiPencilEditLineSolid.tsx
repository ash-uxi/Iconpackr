import React from 'react';

/**
 * PiPencilEditLineSolid icon from the solid style in general category.
 */
interface PiPencilEditLineSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
}

export default function PiPencilEditLineSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'pencil-edit-line icon',
  ...props
}: PiPencilEditLineSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      
       style={{color: color || "currentColor"}}
      
      role="img"
      aria-label={ariaLabel}
      {...props}
    >
      <path d="M13 20C12.4477 20 12 20.4477 12 21C12 21.5523 12.4477 22 13 22H21C21.5523 22 22 21.5523 22 21C22 20.4477 21.5523 20 21 20H13Z" fill="currentColor" stroke="none"/>
  <path d="M19.3694 2.41141C18.3496 1.7525 17.0102 1.89744 16.154 2.75744L3.05189 15.917C2.83564 16.1336 2.62424 16.3454 2.46567 16.601C2.32684 16.8248 2.22287 17.0684 2.15724 17.3234C2.08231 17.6145 2.07531 17.9137 2.06813 18.2205L2.0003 20.9709C1.99366 21.24 2.09582 21.5004 2.28367 21.6933C2.47151 21.8861 2.72919 21.9951 2.9984 21.9955L5.79769 22.0001C6.11408 22.001 6.42451 22.0019 6.72739 21.9292C6.99253 21.8656 7.24588 21.7604 7.47811 21.6176C7.74324 21.4546 7.96201 21.2343 8.18547 21.0093L21.2101 7.92735C22.0392 7.09458 22.2777 5.76573 21.609 4.68846C21.042 3.77496 20.2689 2.99261 19.3694 2.41141Z" fill="currentColor" stroke="none"/>
    </svg>
  );
}
