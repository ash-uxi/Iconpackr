...
/**
 * PiCameraAddDuoStroke icon from the duo-stroke style in devices category.
 */
interface PiCameraAddDuoStrokeProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiCameraAddDuoStroke({
  size = 24,
  color,
  className,
  ariaLabel = 'camera-add icon',
  ...props
}: PiCameraAddDuoStrokeProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
      ...
      {...props}
    >
      <path d="M12 16V13M12 13V10M12 13H9M12 13H15" stroke={color || "currentColor"} strokeWidth="2" fill="none"/>
