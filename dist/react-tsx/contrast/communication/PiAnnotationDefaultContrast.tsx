...
/**
 * PiAnnotationDefaultContrast icon from the contrast style in communication category.
 */
interface PiAnnotationDefaultContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiAnnotationDefaultContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'annotation-default icon',
  ...props
}: PiAnnotationDefaultContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
