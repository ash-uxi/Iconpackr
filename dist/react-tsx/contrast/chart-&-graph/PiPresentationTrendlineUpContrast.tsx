...
/**
 * PiPresentationTrendlineUpContrast icon from the contrast style in chart-&-graph category.
 */
interface PiPresentationTrendlineUpContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiPresentationTrendlineUpContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'presentation-trendline-up icon',
  ...props
}: PiPresentationTrendlineUpContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
