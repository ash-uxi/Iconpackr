...
/**
 * PiFile02PngFormatContrast icon from the contrast style in files-&-folders category.
 */
interface PiFile02PngFormatContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFile02PngFormatContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'file-02-png-format icon',
  ...props
}: PiFile02PngFormatContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
