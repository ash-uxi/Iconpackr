...
/**
 * PiClipboardPrescriptionStroke icon from the stroke style in medical category.
 */
interface PiClipboardPrescriptionStrokeProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardPrescriptionStroke({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-prescription icon',
  ...props
}: PiClipboardPrescriptionStrokeProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
