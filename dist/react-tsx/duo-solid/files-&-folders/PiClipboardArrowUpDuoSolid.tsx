...
/**
 * PiClipboardArrowUpDuoSolid icon from the duo-solid style in files-&-folders category.
 */
interface PiClipboardArrowUpDuoSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardArrowUpDuoSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-arrow-up icon',
  ...props
}: PiClipboardArrowUpDuoSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
