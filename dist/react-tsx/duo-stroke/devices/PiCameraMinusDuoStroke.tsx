...
/**
 * PiCameraMinusDuoStroke icon from the duo-stroke style in devices category.
 */
interface PiCameraMinusDuoStrokeProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiCameraMinusDuoStroke({
  size = 24,
  color,
  className,
  ariaLabel = 'camera-minus icon',
  ...props
}: PiCameraMinusDuoStrokeProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
  ...
  <path d="M9 13H15" stroke={color || "currentColor"} strokeWidth="2" fill="none"/>
    </svg>
  );
}
