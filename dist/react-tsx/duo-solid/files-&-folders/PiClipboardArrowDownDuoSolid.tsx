...
/**
 * PiClipboardArrowDownDuoSolid icon from the duo-solid style in files-&-folders category.
 */
interface PiClipboardArrowDownDuoSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardArrowDownDuoSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-arrow-down icon',
  ...props
}: PiClipboardArrowDownDuoSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
