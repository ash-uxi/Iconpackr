...
/**
 * PiClipboardPrescriptionDuoSolid icon from the duo-solid style in medical category.
 */
interface PiClipboardPrescriptionDuoSolidProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiClipboardPrescriptionDuoSolid({
  size = 24,
  color,
  className,
  ariaLabel = 'clipboard-prescription icon',
  ...props
}: PiClipboardPrescriptionDuoSolidProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
