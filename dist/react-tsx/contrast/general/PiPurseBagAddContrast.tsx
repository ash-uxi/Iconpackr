...
/**
 * PiPurseBagAddContrast icon from the contrast style in general category.
 */
interface PiPurseBagAddContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiPurseBagAddContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'purse-bag-add icon',
  ...props
}: PiPurseBagAddContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
