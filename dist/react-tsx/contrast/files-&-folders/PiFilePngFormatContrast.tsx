...
/**
 * PiFilePngFormatContrast icon from the contrast style in files-&-folders category.
 */
interface PiFilePngFormatContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiFilePngFormatContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'file-png-format icon',
  ...props
}: PiFilePngFormatContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
