import React from 'react';

/**
 * PiTrackpadStroke icon from the stroke style in devices category.
 */
interface PiTrackpadStrokeProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
}

export default function PiTrackpadStroke({
  size = 24,
  color,
  className,
  ariaLabel = 'trackpad icon',
  ...props
}: PiTrackpadStrokeProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      className={className}
      fill="none"
      stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{color: color || "currentColor"}}
      
      role="img"
      aria-label={ariaLabel}
      {...props}
    >
      <path d="M2 13H12M22 13H12M12 13V21" stroke="currentColor" strokeWidth="2" fill="none"/>
  <rect x="2" y="3" width="20" height="18" rx="4" stroke="currentColor" strokeWidth="2" fill="none"/>
    </svg>
  );
}
