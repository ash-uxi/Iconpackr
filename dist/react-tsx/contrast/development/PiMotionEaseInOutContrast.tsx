...
/**
 * PiMotionEaseInOutContrast icon from the contrast style in development category.
 */
interface PiMotionEaseInOutContrastProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  color?: string;
  className?: string;
  ariaLabel?: string;
...
export default function PiMotionEaseInOutContrast({
  size = 24,
  color,
  className,
  ariaLabel = 'motion-ease-in-out icon',
  ...props
}: PiMotionEaseInOutContrastProps): JSX.Element {
  return (
    <svg
      width={size}
      height={size}
